...
function BookingFlightsConfirmDetails(props) {
  ...
  useEffect(() => {
    ...
    var traceId = props.location.state.traceId;
       ...
       console.log("resultIndex",traceId)

    const getToken=async()=>{
    const URL = "https://travelvogues.com/api/authentication";

    const form = new FormData();
    form.append('UserName', 'Anish2702');
    form.append('Password', 'Anish2702@');
    form.append('IP', '14.139.250.131');
    ...
    .then(function (response) {
      ...
      var token=response.data[0].AgentData[0].Token
      console.log(response.data[0].AgentData[0].Token,"data")
      giveToken(token,traceId, resultIndex)
  ...
  const giveToken = (tokenId, traceId, resultIndex) => {
    ...
    const fare = async () => {
      ...
      const URL=`${json.quickaiserver}FareQuote`;
      console.log("hey",tokenId,traceId,resultIndex)
